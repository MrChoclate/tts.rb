...
dict = Set.new
File.read('cmu.dict').each_line do |line|
  dict.add(line.split()[0])
end

def get_book_words(book_path)
  book_words = format_book(File.read(book_path)).to_set
end

todo = Set.new

books = `find data_crawl/en/{phil_chenevert,elizabeth_klett} -name "*.txt"`.split("\n")
